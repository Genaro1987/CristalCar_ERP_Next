...
-- Uses JOIN instead of scalar subquery to avoid LibSQL column-resolution bug
-- where CODIGO_TELA is incorrectly resolved against the target table SEG_PERFIL_TELA.
...
JOIN CORE_TELA T_OLD ON T_OLD.CODIGO_TELA = 'FIN001_PLANO_CONTA' AND A.ID_TELA = T_OLD.ID_TELA
JOIN CORE_TELA T_NEW ON T_NEW.CODIGO_TELA = 'FIN_PLANO_CONTA'
WHERE NOT EXISTS (
    SELECT 1 FROM CORE_AJUDA_TELA X WHERE X.ID_TELA = T_NEW.ID_TELA
  );
